...
var MenuContentImpl_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props, { emit: __emit }) {
		...
		const highlightedElement = (0, vue.ref)();
		function onKeydownNavigation(event) {
			const el = require_shared_useArrowNavigation.useArrowNavigation(event, highlightedElement.value || require_shared_getActiveElement.getActiveElement(), contentElement.value, {
				loop: loop.value,
		...
		const filterElement = (0, vue.ref)();
		const activeSubmenuContext = (0, vue.ref)();
		(0, vue.watch)(highlightedElement, (el) => {
